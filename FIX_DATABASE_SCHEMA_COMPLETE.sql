...
-- This fixes the jobs table ID type and job_offers table to work together
...
-- Step 2: Check if jobs table has INTEGER or UUID ID
SELECT 
  column_name, 
  data_type,
  column_default
...
-- Step 4: Drop job_offers table completely
DROP TABLE IF EXISTS job_offers CASCADE;

-- Step 5: Fix jobs table ID type if it's INTEGER
-- First, let's check what type it actually is
DO $$
DECLARE
    id_type text;
BEGIN
    ...
    IF id_type = 'integer' THEN
        -- Convert jobs.id from INTEGER to UUID
        -- This is a complex operation, so we'll recreate the table
        
        -- Create a backup of existing data
        CREATE TABLE jobs_backup AS SELECT * FROM jobs;
        
        -- Drop and recreate jobs table with UUID
        ...
        -- Restore data (without the old ID)
        INSERT INTO jobs (user_id, company, position, date_applied, status, salary, notes, job_url, job_description, offers, withdrawn, created_at, updated_at)
        SELECT user_id, company, position, date_applied, status, salary, notes, job_url, job_description, offers, withdrawn, created_at, updated_at
        FROM jobs_backup;
...
-- Step 6: Create job_offers table with correct UUID reference
